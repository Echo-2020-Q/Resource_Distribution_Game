...
class SpatialGame:
    ...
    def run(self) -> Dict[str, List[float]]:
        ...
        for t in range(1, self.p.T + 1):
            ...
            self.res += round_payoff

            # (2) 统一消耗
            # 对低资源个体：减掉固定cost；对高资源个体：乘beta
            # (2) 统一消耗：条件规则
            cost = self.p.gamma  # 固定阈值
            mask = self.res <= self.p.gamma  # 哪些个体资源低于等于 α
            self.res = np.where(mask, self.res - cost, (1-self.p.beta) * self.res)
