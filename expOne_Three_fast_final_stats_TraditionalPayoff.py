...
class SpatialGameFast:
    def __init__(self, p: Params):
        ...
        self.L = p.Lsize
        self.include_loner = (p.fL is not None) or p.allow_forced_l_in_cd
    ...
    def _force_loners_if_broke(self):
        ...
        if np.any(broke):
            need_force = broke & (self.prev_strat != L)
            if self.include_loner and self.p.allow_forced_l_in_cd:
                self.strat[need_force] = L
                self.just_forced_L[need_force] = True
            # 不论是否允许出现 L，都把破产者资源钉 0
            #self.res[broke] = 0.0
    ...
    def run(self) -> Dict[str, float]:
        ...
        for t in range(1, self.p.T + 1):
            ...
            if self.p.learn_signal == "round":
                ...
                # === 全口径统计：凡是因 L 存在带来的所有 σ 都算入补贴 ===
                maskL = (self.strat == L)
                # 每个格子的 L 邻居个数（0..4）
                L_up, L_down, L_left, L_right = rolled(maskL.astype(np.int8))
                L_nb_cnt = L_up + L_down + L_left + L_right

                # 发给 L 本人的：每个 L 与 4 个邻居交互都拿 σ → 4 * nL * σ
                nL = int(maskL.sum())
                sigma_to_L = 4.0 * nL * self.p.sigma

                # 发给非 L 的：非 L 每遇到一个 L 邻居也拿 σ → sum_{非L}(L_nb_cnt) * σ
                sigma_to_nonL = float(np.sum((~maskL) * L_nb_cnt)) * self.p.sigma

                self.cum_sigma += (sigma_to_L + sigma_to_nonL)
...
if __name__ == "__main__":
    p_cd = Params(
        ...
        pi0 = 5,                           # 初始资源
        r = 1.6, sigma = 0.25,             # r: 背叛诱惑系数, sigma: Loner低保
        kappa = 1,                         # 费米学习因子
        learn_signal = "cumulative",       # 'per_strategy' / 'round' / 'cumulative'
        seed = 0,
        allow_forced_l_in_cd = False       # 资源耗尽者是否变为 L
    )
